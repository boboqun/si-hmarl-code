...
scales = [("500m x 500m", 0.25), ("1km x 1km", 0.5)]
n_seeds = 10
...
with open(out_file, 'a') as f:
    ...
    for i, m in enumerate(makespans):
        writer.writerow([i, scale_name, m, 1.0000, "Success"])
ray.shutdown()
"""

for name, mult in scales:
    temp_dir = tempfile.mkdtemp(prefix=f"route_sandbox_{mult}_")
    ...
    with open(script_path, 'w') as f:
        f.write(eval_script_template.format(scale_name=name, scale_mult=mult))
